...
const toast = useToast(), busy = ref(''), rejectTarget = ref<PendingBatch | null>(null), conflict = ref(''), sourceId = ref<string | null>(null)
...
function openSource(batch: PendingBatch) {
  sourceId.value = batch.entry_id || null
}
onMounted(() => load())
</script>
<template>
  <div class="page stack">
    ...
    <p v-if="conflict" role="alert" class="card">{{ conflict }} <button class="link" @click="load(true)">Загрузить актуальную версию</button></p>
    ...
    <article v-for="batch in data?.batches" v-else :key="batch.id" class="card stack">
      ...
      <div class="cluster">
        ...
        <UiButton v-if="batch.entry_id" variant="ghost" @click="openSource(batch)">Показать исходную запись</UiButton>
      </div>
    </article>
