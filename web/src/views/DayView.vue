...
const {data,loading,error,load}=useAsyncState(async()=>{try{return await journalApi.day(date)}catch{const list=await journalApi.events(`?from=${date}&to=${date}`);return{date,events:list.events}}})
...
<template>
  <div class="page stack">
    ...
    <StatePanel v-else-if="!data?.events.length" kind="empty" title="За этот день нет записей"/>
    <EventCard v-for="event in data?.events" v-else :key="event.id" :event="event" actions @delete="target=$event" @source="sourceId=$event"/>
    <UiDialog :open="!!target" title="Удалить событие?" @close="target=null">
      <p>Событие исчезнет из дневника и аналитики. Исходная запись хранится отдельно.</p>
      <div class="cluster">
        <UiButton variant="danger" :busy="busy" @click="remove">Удалить</UiButton>
        <UiButton variant="secondary" @click="target=null">Отмена</UiButton>
      </div>
    </UiDialog>
